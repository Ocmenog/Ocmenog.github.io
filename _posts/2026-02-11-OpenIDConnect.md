...
## Description
OpenID Connect (OIDC) est un protocole permettant l'authentification d'un utilisateur à un service par un tiers de confiance.
OpenID Connect se base en fait sur OAuth 2.0 pour gérer l'autorisation, en ajoutant une couche d'authentification.
...
### Jetons
Jeton d'accès : Jeton permettant l'accès aux ressources.
...
Jeton de rafraîchissement : Jeton permettant le renouvellement de l'access token.
...
## Fonctionnement
En résumé, l'OIDC va intervenir quand un utilisateur va vouloir se connecter à une application (client).
...
5. Le client s'authentifie à l'AS, en backchannel, et fournit le code d'autorisation pour récupérer les jetons d'identification et d'accès
6. L'AS transmet les jetons au client dans un JWT
